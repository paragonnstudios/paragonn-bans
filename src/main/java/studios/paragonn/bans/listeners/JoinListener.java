...
import studios.paragonn.bans.sync.Packet;
...
public class JoinListener extends ListenerSkeleton
{
    ...
    public void onEnter(final PlayerJoinEvent e) {
        if (PBans.instance.isBungee()) {
            Bukkit.getScheduler().scheduleSyncDelayedTask((Plugin)PBans.instance, (Runnable)new Runnable() {
                public void run() {
                    ...
                    e.getPlayer().sendPluginMessage((Plugin)PBans.instance, "BungeeCord", b.toByteArray());
                }
            });
        }
    }
