...
public class Msg
{
	...
	public static void reload() {
        final File f = new File(PBans.instance.getDataFolder(), "messages.yml");
        Msg.cfg = new YamlConfiguration();
        try {
            final YamlConfiguration defaults = new YamlConfiguration();
            InputStream in = PBans.instance.getResource("messages.yml");
            defaults.load(in);
            in.close();
            if (f.exists()) {
                Msg.cfg.load(f);
                ...
                in = PBans.instance.getResource("messages.yml");
            ...
            try {
                Msg.cfg.load(PBans.instance.getResource("messages.yml"));
            }
            catch (Exception ex) {
                ex.printStackTrace();
            }
